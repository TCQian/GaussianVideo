...
__global__ void rasterize_backward_sum_kernel_video(
    ...
    const float6* __restrict__ conics,   // Full conic parameters (float6)
    ...
    float3* __restrict__ v_xy,           // Gradient wrt (x,y) for each Gaussian (output)
    float6* __restrict__ v_conic,        // Gradient wrt conic parameters (all 6 components)
    ...
    // Compute voxel indices within the 3D volume:
    auto block = cg::this_thread_block();
    ...
    const float px = (float)j;
    const float py = (float)i;
    const float pz = (float)k;
    // Linear index for 3D image (with bounds clamped)
    const int32_t pix_id = min((i * img_size.x + j) + (k * img_size.x * img_size.y),
    ...
    __shared__ int32_t id_batch[BLOCK_SIZE];
    __shared__ float4 xyz_opacity_batch[BLOCK_SIZE];  // Stores (x, y, z, opacity)
    ...
    cg::thread_block_tile<32> warp = cg::tiled_partition<32>(block);
    int warp_bin_final = cg::reduce(warp, bin_final, cg::greater<int>());
    int tr = block.thread_rank();

    // Loop over batches of gaussians in this tile:
    for (int b = 0; b < num_batches; ++b) {
        block.sync();
        const int batch_end = range.y - 1 - BLOCK_SIZE * b;
        int batch_size = min(BLOCK_SIZE, batch_end + 1 - range.x);
        ...
        if (idx >= range.x) {
            ...
            conic_batch[tr] = conics[g_id];  // Now full float6
            rgbs_batch[tr] = rgbs[g_id];
        }
        block.sync();
        ...
        for (int t = max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            float3 delta;
            float vis;
            // Declare a local copy of the full conic (float6):
            float6 conic;
            if (valid) {
                conic = conic_batch[t];
                float4 xy_opac = xyz_opacity_batch[t];
                opac = xy_opac.w;
                // Compute delta between Gaussian center and voxel center:
                delta = { xy_opac.x - px, xy_opac.y - py, xy_opac.z - pz };
            ...
            // Compute gradients for valid gaussians:
            float3 v_rgb_local = { 0.f, 0.f, 0.f };
            float6 v_conic_local = { 0.f, 0.f, 0.f, 0.f, 0.f, 0.f }; // full gradient
            ...
            if (valid) {
                ...
                // Here we follow the same form as the forward step for v_xy:
                // (Note: The full derivative would include terms from all 3 coordinates,
                // but we only update the (x,y) gradient as v_xy is float3.)
                v_xy_local.x = v_sigma * (conic.x * delta.x + conic.y * delta.y + conic.z * delta.z);
                v_xy_local.y = v_sigma * (conic.y * delta.x + conic.w * delta.y + conic.u * delta.z);
                v_xy_local.z = v_sigma * (conic.z * delta.x + conic.u * delta.y + conic.v * delta.z);

                v_opacity_local = vis * v_alpha;
            }
            // Warp-level reductions:
            warpSum3(v_rgb_local, warp);
            warpSum(v_conic_local.x, warp);
            warpSum(v_conic_local.y, warp);
            warpSum(v_conic_local.z, warp);
            warpSum(v_conic_local.w, warp);
            warpSum(v_conic_local.u, warp);
            warpSum(v_conic_local.v, warp);
            warpSum3(v_xy_local, warp);
            warpSum(v_opacity_local, warp);
            
            if (warp.thread_rank() == 0) {
                int32_t g = id_batch[t];
                // Update gradients for v_rgb:
                atomicAdd(&(v_rgb[g].x), v_rgb_local.x);
                atomicAdd(&(v_rgb[g].y), v_rgb_local.y);
                atomicAdd(&(v_rgb[g].z), v_rgb_local.z);
                // Update gradients for v_conic (all 6 components):
                atomicAdd(&(v_conic[g].x), v_conic_local.x);
                atomicAdd(&(v_conic[g].y), v_conic_local.y);
                atomicAdd(&(v_conic[g].z), v_conic_local.z);
                atomicAdd(&(v_conic[g].w), v_conic_local.w);
                atomicAdd(&(v_conic[g].u), v_conic_local.u);
                atomicAdd(&(v_conic[g].v), v_conic_local.v);
                // Update gradients for v_xy:
                atomicAdd(&(v_xy[g].x), v_xy_local.x);
                atomicAdd(&(v_xy[g].y), v_xy_local.y);
                atomicAdd(&(v_xy[g].z), v_xy_local.z);
                // Update gradient for v_opacity:
                atomicAdd(v_opacity + g, v_opacity_local);
            }
        }
    }
