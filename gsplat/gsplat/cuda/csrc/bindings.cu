...
> rasterize_forward_sum_video_tensor(
    ...
    const torch::Tensor &gaussian_ids_sorted,
    const torch::Tensor &tile_bins,
    const torch::Tensor &xyzs,        // Updated to 3D coordinates
    const torch::Tensor &conics,
    const torch::Tensor &colors,
    const torch::Tensor &opacities,
    const torch::Tensor &background
    ...
    CHECK_INPUT(gaussian_ids_sorted);
    CHECK_INPUT(tile_bins);
    CHECK_INPUT(xyzs);                // Updated to xyzs
    CHECK_INPUT(conics);
    CHECK_INPUT(colors);
    CHECK_INPUT(opacities);
    CHECK_INPUT(background);

    // Set tile bounds as 3D
    dim3 tile_bounds_dim3;
    tile_bounds_dim3.x = std::get<0>(tile_bounds);
    tile_bounds_dim3.y = std::get<1>(tile_bounds);
    tile_bounds_dim3.z = std::get<2>(tile_bounds);

    // Set block dimensions as 3D
    dim3 block_dim3;
    block_dim3.x = std::get<0>(block);
    block_dim3.y = std::get<1>(block);
    block_dim3.z = std::get<2>(block);

    // Set image dimensions as 3D
    dim3 img_size_dim3;
    img_size_dim3.x = std::get<0>(img_size);
    img_size_dim3.y = std::get<1>(img_size);
    img_size_dim3.z = std::get<2>(img_size);
    ...
    const int img_depth = img_size_dim3.z;
    const int img_height = img_size_dim3.y;
    const int img_width = img_size_dim3.x;
    const int channels = colors.size(1);

    // Update output tensors for 3D
    torch::Tensor out_img = torch::zeros(
        {img_depth, img_height, img_width, channels}, xyzs.options().dtype(torch::kFloat32)
    );

    torch::Tensor final_Ts = torch::zeros(
        {img_depth, img_height, img_width}, xyzs.options().dtype(torch::kFloat32)
    );
    ...
    rasterize_forward_sum_video<<<tile_bounds_dim3, block_dim3>>>(
        ...
        gaussian_ids_sorted.contiguous().data_ptr<int32_t>(),
        (int2 *)tile_bins.contiguous().data_ptr<int>(),
        (float3 *)xyzs.contiguous().data_ptr<float>(),        // Cast xyzs to float3* for 3D positions
        (float6 *)conics.contiguous().data_ptr<float>(),
        (float3 *)colors.contiguous().data_ptr<float>(),
        opacities.contiguous().data_ptr<float>(),
        final_Ts.contiguous().data_ptr<float>(),
...
std::tuple<
    ...
    torch::Tensor, // dL_dcolors
    torch::Tensor  // dL_dopacity
> rasterize_backward_sum_video_tensor(
    const unsigned img_height,
    const unsigned img_width,
    const unsigned img_depth,          // New depth parameter for 3D support
    const unsigned BLOCK_H,
    const unsigned BLOCK_W,
    const unsigned BLOCK_T,
    const torch::Tensor &gaussians_ids_sorted,
    const torch::Tensor &tile_bins,
    const torch::Tensor &xyzs,         // Updated to 3D coordinates
    const torch::Tensor &conics,
    ...
    // std::cout << "[DEBUG] Launching rasterize_backward_sum_kernel_video" << std::endl;
    // std::cout << "[DEBUG] v_output - Min: " << v_output.min().item<float>() 
    //           << ", Max: " << v_output.max().item<float>() 
    //           << ", Mean: " << v_output.mean().item<float>() << std::endl;

    // Check inputs
    CHECK_INPUT(xyzs);                 // Updated to check 3D coordinates
    CHECK_INPUT(colors);

    // Verify xyzs dimensions for 3D
    if (xyzs.ndimension() != 2 || xyzs.size(1) != 3) {
        AT_ERROR("xyzs must have dimensions (num_points, 3)");
    }

    // Verify colors dimensions
    if (colors.ndimension() != 2 || colors.size(1) != 3) {
        AT_ERROR("colors must have 2 dimensions");
    }
    ...
    const dim3 tile_bounds = {
        (img_width + BLOCK_W - 1) / BLOCK_W,
        (img_height + BLOCK_H - 1) / BLOCK_H,
        (img_depth + BLOCK_T - 1) / BLOCK_T
    };
    const dim3 block(BLOCK_W, BLOCK_H, BLOCK_T); // Block size now includes depth
    const dim3 img_size = {img_width, img_height, img_depth}; // Image size now 3D
    const int channels = colors.size(1);

    // Allocate output tensors for 3D gradients
    torch::Tensor v_xy = torch::zeros({num_points, 3}, xyzs.options());      
    torch::Tensor v_conic = torch::zeros({num_points, 6}, xyzs.options());
    torch::Tensor v_colors = torch::zeros({num_points, channels}, xyzs.options());
    torch::Tensor v_opacity = torch::zeros({num_points, 1}, xyzs.options());

    // Launch the 3D kernel
    rasterize_backward_sum_kernel_video<<<tile_bounds, block>>>(
        tile_bounds,
        img_size,
        gaussians_ids_sorted.contiguous().data_ptr<int>(),
        (int2 *)tile_bins.contiguous().data_ptr<int>(),     
        (float3 *)xyzs.contiguous().data_ptr<float>(),      
        (float6 *)conics.contiguous().data_ptr<float>(),
        (float3 *)colors.contiguous().data_ptr<float>(),
        opacities.contiguous().data_ptr<float>(),
        *(float3 *)background.contiguous().data_ptr<float>(),
    ...
    // std::cout << "[DEBUG] Finished rasterization backward kernel" << std::endl;

    //  // Print v_conic after kernel execution
    //  auto v_conic_cpu = v_conic.to(torch::kCPU);
    //  std::cout << "[DEBUG] v_conic - Min: " << v_conic_cpu.min().item<float>() 
    //            << ", Max: " << v_conic_cpu.max().item<float>() 
    //            << ", Mean: " << v_conic_cpu.mean().item<float>() << std::endl;
 
    //  // Print first few elements for manual checking
    //  for (int i = 0; i < std::min(10, num_points); ++i) {
    //      auto c = v_conic_cpu[i];
    //      std::cout << "[DEBUG] v_conic[" << i << "]: (" 
    //                << c[0].item<float>() << ", " << c[1].item<float>() << ", " 
    //                << c[2].item<float>() << ", " << c[3].item<float>() << ", " 
    //                << c[4].item<float>() << ", " << c[5].item<float>() << ")" << std::endl;
    //  }
