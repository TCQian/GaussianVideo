...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("project_gaussians_video_forward", &project_gaussians_video_forward_tensor);
    m.def("project_gaussians_video_backward", &project_gaussians_video_backward_tensor);
    // utils
    ...
    m.def("get_tile_bin_edges_video", &get_tile_bin_edges_video_tensor);

    m.def("map_gaussian_to_intersects_video", &map_gaussian_to_intersects_video_tensor);

    m.def("rasterize_sum_forward_video", &rasterize_forward_sum_video_tensor);
    m.def("rasterize_sum_backward_video", &rasterize_backward_sum_video_tensor);
}
